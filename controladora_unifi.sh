...
dpkg -l | grep "mongo-tools\\|mongodb\\|mongodb-org" | grep -v "mongodb-database-tools" | grep "^ii\\|^hi" | awk '{print $3}' | sed 's/.*://' | sed 's/-.*//g' &> /tmp/EUS/mongodb_versions
...
if [[ "${mongo_version_supported}" == "4.4.999" ]]; then
  ...
  libssl_grep_arg="libssl1.1.1.*amd64.deb"
  libssl_arm64_url="https://launchpad.net/~ubuntu-security/+archive/ubuntu/ppa/+build/26217085/+files/openssl_1.1.1f-1ubuntu2.19_arm64.deb"
  if [[ "${os_codename}" =~ (buster|bullseye|bookworm|trixie|forky) ]]; then
  ...
  libssl_grep_arg="libssl1.0.0.*amd64.deb"
...
mongodb_44_key() {
  echo -e "${WHITE_R}#${RESET} Adding key for MongoDB 4.4..."
  aptkey_depreciated
  if [[ "${apt_key_deprecated}" == 'true' ]]; then
    if wget -qO - https://www.mongodb.org/static/pgp/server-4.4.asc | gpg --dearmor | tee -a /usr/share/keyrings/mongodb-server-4.4.gpg &> /dev/null; then echo -e "${GREEN}#${RESET} Successfully added key for MongoDB 4.4! \\n"; signed_by_value=" signed-by=/usr/share/keyrings/mongodb-server-4.4.gpg"; else abort; fi
  else
    if wget -qO - https://www.mongodb.org/static/pgp/server-4.4.asc | apt-key add - &> /dev/null; then echo -e "${GREEN}#${RESET} Successfully added key for MongoDB 4.4! \\n"; else abort; fi
  fi
}

if [[ "${os_codename}" =~ (disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar|mantic) && "${architecture}" =~ (amd64|arm64) ]]; then
  header
  ...
  if [[ "${architecture}" == "amd64" ]]; then
    libssl_url=$(curl -s http://security.ubuntu.com/ubuntu/pool/main/o/${libssl_url_arg}/ | grep -io "${libssl_grep_arg}" | sed '/u5_/d' | cut -d'"' -f1 | tail -n1)
    echo -e "${WHITE_R}#${RESET} Downloading libssl..."
    if wget "${wget_progress[@]}" -O "$libssl_temp" "http://security.ubuntu.com/ubuntu/pool/main/o/${libssl_url_arg}/${libssl_url}" &>> "${eus_dir}/logs/libssl.log"; then echo -e "${GREEN}#${RESET} Successfully downloaded libssl!"; else echo -e "${RED}#${RESET} Failed to download libssl..."; abort; fi
  fi
...
if [[ "${mongodb_installed}" != 'true' ]]; then
  ...
  if [[ "${os_codename}" =~ (precise|maya|trusty|qiana|rebecca|rafaela|rosa|xenial|sarah|serena|sonya|sylvia) && ! "${architecture}" =~ (amd64|arm64) ]]; then
  ...
  elif [[ "${os_codename}" =~ (bionic|tara|tessa|tina|tricia|disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar|mantic) && "${architecture}" == "i386" ]]; then
    ubuntu_32_mongo
    ...
    rm --force "$libssl_temp" 2> /dev/null
    if [[ "${os_codename}" =~ (disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar|mantic) ]]; then
      if [[ "${archived_repo}" != "true" ]]; then
    ...
    mongo_tools_url=$(curl -s http://archive.debian.org/debian/pool/main/m/mongo-tools/ | grep -io "mongo-tools_3.2.*i386.deb" | sed 's/&#43;/+/g' | tail -n1)
    ...
    mongodb_clients_url=$(curl -s http://archive.debian.org/debian/pool/main/m/mongodb/ | grep -io "mongodb-clients_3.2.*i386.deb" | sed 's/&#43;/+/g' | tail -n1)
    ...
    mongodb_server_url=$(curl -s http://archive.debian.org/debian/pool/main/m/mongodb/ | grep -io "mongodb-server_3.2.*i386.deb" | sed 's/&#43;/+/g' | tail -n1)
  ...
  elif [[ "${os_codename}" =~ (xenial|bionic|cosmic|disco|eoan|focal|groovy|hirsute|impish|jammy|kinetic|lunar|mantic|sarah|serena|sonya|sylvia|tara|tessa|tina|tricia) && "${architecture}" =~ (amd64|arm64) ]]; then
    if [[ "${mongo_version_supported}" == "4.4.999" ]]; then
      mongodb_44_key
    elif [[ "${mongo_version_supported}" == "3.6.999" ]]; then
      mongodb_36_key
    ...
    echo "deb [ arch=amd64,arm64${signed_by_value} ] https://repo.mongodb.org/apt/${mongodb_codename}/mongodb-org/${mongo_version_supported::3} ${mongodb_repo_type}" &> "/etc/apt/sources.list.d/mongodb-org-${mongo_version_supported::3}.list" || abort
    hide_apt_update=true
    run_apt_get_update
    echo -e "${WHITE_R}#${RESET} Installing mongodb-org version ${mongo_version_supported::3}..."
    if DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install mongodb-org &>> "${eus_dir}/logs/mongodb_install.log"; then echo -e "${GREEN}#${RESET} Successfully installed mongodb-org version ${mongo_version_supported::3}! \\n"; else echo -e "${RED}#${RESET} Failed to install mongodb-org version ${mongo_version_supported::3}...\\n"; abort; fi
...
if [[ "${architecture}" == "armhf" ]] && [[ "${mongodb_installed}" != 'true' ]]; then
  header
...
if ! dpkg -l | grep "^ii\\|^hi" | grep -iq "mongodb-server\\|mongodb-org-server"; then dpkg_ignore_depends_flag="--ignore-depends=mongodb-server"; fi
