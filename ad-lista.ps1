...
$analista = "Jhonathan Chaves" # Nome do Analista
$empresa = "BNP Solu&ccedil;&otilde;es em TI" # Nome da Empresa
...
$usuarios = @(Get-ADUser -filter * -Properties SamAccountName, Name, PasswordNeverExpires, Enabled, Created, MemberOf)
# Lista todas as Propriedades do Usuário
#PS C:\AD> Get-ADUser marcos -Properties *

$resultado = @($usuarios | Select-Object SamAccountName, Name, PasswordNeverExpires, Enabled, Created, @{n="MemberOf";e={[string]$_.MemberOf}})

# Ordenar pela Empresa (Company) A-Z
$resultado = $resultado | Sort "Name" 
...
$titulo=
		...
		<tr>
		<td bgcolor='#F9F9F9'>
		<font face='Calibri' size='10px'>Active Directory - Lista de Usu&aacute;rios - Mitsui</font>
		<H3 align='center'>Empresa: $empresa - Dom&iacute;nio: $dominio - Relat&oacute;rio: $data - Respons&aacute;vel: $analista
		</H3>
		</td>
		</tr>
		</table>
